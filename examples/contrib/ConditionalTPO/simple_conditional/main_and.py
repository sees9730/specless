"""
Conditional TPO — AND condition example
========================================

5 nodes: depot(0), e1(1), e2(2), e3(3), e4(4)

Precedence: 1->2, 1->3, 1->4
Mandatory:  depot, e1, e2, e3
Optional:   e4 — visit only if BOTH e2 AND e3 are visited

Activation (from paper Def 5, linearized AND):
  z2  = 1 iff e2 visited
  z3  = 1 iff e3 visited
  z_e4 = z2 AND z3:
    z_e4 <= z2
...
Conditional TPO: if z_e4=1 → e3->e4 in [0,5]
...
Expected: depot->e1->e2->e3->e4->depot  (z2=1, z3=1, z_e4=1)
"""
...
z2  = m.addVar(vtype=GRB.BINARY, name="z2")   # e2 visited
z3  = m.addVar(vtype=GRB.BINARY, name="z3")   # e3 visited
ze4 = m.addVar(vtype=GRB.BINARY, name="ze4")  # e2 AND e3 visited
...
# --- AND linearization: ze4 = z2 AND z3 (paper Def 5) ---
m.addConstr(ze4 <= z2,           "and_lb1")
m.addConstr(ze4 <= z3,           "and_lb2")
m.addConstr(ze4 >= z2 + z3 - 1, "and_lb3")
...
if m.status != GRB.OPTIMAL:
    ...
    viz_labels = {0: "depot", 1: "e_1", 2: "e_2", 3: "e_3", 4: "e_4"}
    plot_ctpo(
        "visualization/ctpo_and.png",
        labels=viz_labels,
        mandatory={0, 1, 2, 3},
        optional={4},
        ap_nodes=set(),
        ...
        title="cTPO — AND: e_4 if e_2 AND e_3 visited",
    )
    plot_tsp(
        "visualization/tsp_and.png",
        labels=viz_labels,
        mandatory={0, 1, 2, 3},
        optional={4},
        ap_nodes=set(),
