"""
RViz2 Tour Visualizer for Mars Rover Scenario
...
def visualize_tour_rviz(
    ...
    # Build event node list for markers
    event_nodes = []
    ...
    # Collect lava tile positions from the transition system for terrain rendering
    lava_positions = []
    for node_id, data in transition_system.nodes(data=True):
        obs = data.get("observation", "")
        if obs == "lava_red":
            state = node_id if isinstance(node_id, (list, tuple)) else node_id
            # States are tuples (x, y, ...)
            if hasattr(node_id, '__iter__') and not isinstance(node_id, str):
                lava_positions.append([float(node_id[0]), float(node_id[1])])
    ...
    rviz_config = os.path.join(script_dir, "visualization", "mars_rover.rviz")
